...
async def test_expose_unexpose():
    async with base.CleanModel() as model:
        ...
        assert exposed_endpoints == {
            "": {"expose-to-cidrs": ["0.0.0.0/0", "::/0"], "expose-to-spaces": None}
        }

        # Expose all opened port ranges to the CIDRs that correspond to a list
        # of spaces.
        await app.expose(exposed_endpoints={"": ExposedEndpoint(to_spaces=["alpha"])})
        exposed_endpoints = await get_exposed_endpoints(model, app)
        assert exposed_endpoints == {
            "": {"expose-to-cidrs": None, "expose-to-spaces": ["alpha"]}
        }
        ...
        exposed_endpoints = await get_exposed_endpoints(model, app)
        assert exposed_endpoints == {
            "": {"expose-to-cidrs": ["10.0.0.0/24"], "expose-to-spaces": None}
        }
        ...
        exposed_endpoints = await get_exposed_endpoints(model, app)
        assert exposed_endpoints == {
            "": {"expose-to-spaces": ["alpha"], "expose-to-cidrs": ["10.0.0.0/24"]}
        }
        ...
        assert exposed_endpoints == {
            "": {"expose-to-spaces": ["alpha"], "expose-to-cidrs": ["10.0.0.0/24"]},
            "ubuntu": {"expose-to-cidrs": ["10.42.42.0/24"], "expose-to-spaces": None},
        }
