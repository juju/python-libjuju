...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("1.21-alpha1.1", Number(major=1, minor=21, patch=1, tag="alpha", build=1)),
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            {"number": {"major": 1}, "series": "trusty", "arch": "amd64"},
            Binary(Number(1), "trusty", "amd64"),
        ),
        ("1.2.3-trusty-amd64", Binary(Number(1, 2, 3, "", 0), "trusty", "amd64")),
        ("1.2.3.4-trusty-amd64", Binary(Number(1, 2, 3, "", 4), "trusty", "amd64")),
        (
            "1.2-alpha3-trusty-amd64",
            Binary(Number(1, 2, 3, "alpha", 0), "trusty", "amd64"),
        ),
        (
            "1.2-alpha3.4-trusty-amd64",
            Binary(Number(1, 2, 3, "alpha", 4), "trusty", "amd64"),
        ),
        ("1.2.3", TypeError),
        ("1.2-beta1", TypeError),
