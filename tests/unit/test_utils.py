...
class TestSeriesSelector(unittest.TestCase):
    def test_series_arg(self):
        ...
        assert (
            series_selector(series_arg="foo", supported_series=[], force=True) == "foo"
        )

    def test_charm_url(self):
        assert (
            series_selector(
                charm_url=URL.parse("ch:jammy/ubuntu"), supported_series=["jammy"]
            )
            == "jammy"
        )
    ...
    def test_model_config(self):
        ...
        assert (
            series_selector(model_config=mconf, supported_series=["jammy"]) == "jammy"
        )
...
class TestBaseChannelOriginUtils(unittest.TestCase):
    ...
    def test_base_channel_from_series(self):
        assert (
            base_channel_from_series(track="latest", risk="stable", series="jammy")
            == "22.04/stable"
        )
    ...
    def test_get_base_from_series(self):
        ...
        assert get_base_from_origin_or_channel(orgn, series="jammy") == client.Base(
            "22.04/edge", "ubuntu"
        )
...
class TestShouldUpgradeResource(unittest.TestCase):
    def test_should_upgrade_resource_no_same_rev(self):
        ...
        res = {
            "created-at": "2019-10-24T20:45:19.201000",
            "description": "The policy.d overrides file",
            "download": {
                "hash-sha-256": "e3b0c4",
                "hash-sha-384": "38b060a751ac914898b95b",
            ...
            "name": "policyd-override",
            "revision": 0,
            "type": "file",
        }

        existing = {
            "policyd-override": client.Resource(
                charmresource=None,
                application="keystone",
                id_="keystone/policyd-override",
                pending_id="",
