...
class TestControllerConnect(unittest.IsolatedAsyncioTestCase):
    ...
    @mock.patch("juju.client.connector.Connector.connect_controller")
    ...
    @mock.patch("juju.controller.Controller.update_endpoints")
    @mock.patch("juju.client.connector.Connector.connect_controller")
    async def test_with_controller_name(
        self, mock_connect_controller, mock_update_endpoints
    ):
        c = Controller()
    ...
    @mock.patch("juju.controller.Controller.update_endpoints")
    @mock.patch("juju.client.connector.Connector.connect")
    async def test_with_endpoint_and_userpass(
        self, mock_connect, mock_update_endpoints
    ):
        c = Controller()
        with self.assertRaises(TypeError):
            await c.connect(endpoint="0.1.2.3:4566", username="dummy")
        await c.connect(endpoint="0.1.2.3:4566", username="user", password="pass")
        mock_connect.assert_called_once_with(
            endpoint="0.1.2.3:4566", username="user", password="pass"
        )
        mock_update_endpoints.assert_called_once_with()

    @mock.patch("juju.controller.Controller.update_endpoints")
    @mock.patch("juju.client.connector.Connector.connect")
    async def test_with_endpoint_and_bakery_client(
        self, mock_connect, mock_update_endpoints
    ):
        c = Controller()
        await c.connect(endpoint="0.1.2.3:4566", bakery_client="bakery")
        mock_connect.assert_called_once_with(
            endpoint="0.1.2.3:4566", bakery_client="bakery"
        )
        mock_update_endpoints.assert_called_once_with()

    @mock.patch("juju.controller.Controller.update_endpoints")
    @mock.patch("juju.client.connector.Connector.connect")
    async def test_with_endpoint_and_macaroons(
        self, mock_connect, mock_update_endpoints
    ):
        c = Controller()
        await c.connect(endpoint="0.1.2.3:4566", macaroons=["macaroon"])
        mock_connect.assert_called_with(endpoint="0.1.2.3:4566", macaroons=["macaroon"])
        mock_update_endpoints.assert_called_with()
        await c.connect(
            endpoint="0.1.2.3:4566", bakery_client="bakery", macaroons=["macaroon"]
        )
    ...
    @mock.patch("juju.client.connector.Connector.connect_controller")
    @mock.patch("juju.client.connector.Connector.connect")
    async def test_with_posargs(
        self, mock_connect, mock_connect_controller, mock_update_endpoints
    ):
        c = Controller()
        await c.connect("foo")
        mock_connect_controller.assert_called_once_with("foo")
    ...
    @mock.patch("juju.client.client.CloudFacade")
    async def test_file_cred_v2(self, mock_cf):
        with NamedTemporaryFile() as tempfile:
            ...
            cred = client.CloudCredential(
                auth_type="jsonfile", attrs={"file": tempfile.name}
            )
            jujudata = mock.MagicMock()
            c = Controller(jujudata=jujudata)
            c._connector = mock.Mock()
            up_creds = mock.AsyncMock()
