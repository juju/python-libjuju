...
class TestObserver(unittest.TestCase):
    ...
    def test_cares_about_id(self):
        ...
        o = self._make_observer(None, None, None, id_, None)

        delta = _make_delta("application", "change", dict(name=id_))
    ...
    def test_cares_about_type(self):
        ...
        o = self._make_observer(None, type_, None, None, None)

        delta = _make_delta(type_, "change", dict(name="foo"))
    ...
    def test_cares_about_action(self):
        ...
        o = self._make_observer(None, None, action, None, None)

        delta = _make_delta("application", action, dict(name="foo"))
    ...
    def test_cares_about_predicate(self):
        ...
        o = self._make_observer(None, None, None, None, predicate)

        delta = _make_delta("application", "change", dict(fizz="bang"))
...
class TestModelState(unittest.TestCase):
    def test_apply_delta(self):
        ...
        self.assertEqual(len(model.state.state[delta.entity][delta.get_id()]), 1)
        ...
        self.assertEqual(len(model.state.state[delta.entity][delta.get_id()]), 3)
        self.assertIsInstance(new, Application)
...
class TestModelConnect(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_with_endpoint_and_uuid_with_userpass(self, mock_connect, _):
        m = Model()
        with self.assertRaises(TypeError):
            await m.connect(endpoint="0.1.2.3:4566", uuid="some-uuid", username="user")
        await m.connect(
            endpoint="0.1.2.3:4566", uuid="some-uuid", username="user", password="pass"
        )
    ...
    async def test_with_endpoint_and_uuid_with_bakery(self, mock_connect, _):
        m = Model()
        await m.connect(
            endpoint="0.1.2.3:4566", uuid="some-uuid", bakery_client="bakery"
        )
        mock_connect.assert_called_once_with(
            endpoint="0.1.2.3:4566", uuid="some-uuid", bakery_client="bakery"
        )
    ...
    async def test_with_endpoint_and_uuid_with_macaroon(self, mock_connect, _):
        ...
        with self.assertRaises(TypeError):
            await m.connect(endpoint="0.1.2.3:4566", uuid="some-uuid", username="user")
        await m.connect(
            endpoint="0.1.2.3:4566", uuid="some-uuid", macaroons=["macaroon"]
        )
        mock_connect.assert_called_with(
            endpoint="0.1.2.3:4566", uuid="some-uuid", macaroons=["macaroon"]
        )
        await m.connect(
            endpoint="0.1.2.3:4566",
            uuid="some-uuid",
    ...
    async def test_with_posargs(self, mock_connect, mock_connect_model, _):
        ...
        mock_connect.assert_called_once_with(
            endpoint="endpoint", uuid="uuid", username="user", password="pass"
        )
        await m.connect(
            "endpoint",
            "uuid",
...
class TestModelWaitForIdle(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_wait_for_active_status(self):
        ...
        with patch.object(
            Model, "applications", new_callable=PropertyMock
        ) as mock_apps:
            ...
            await m.wait_for_idle(
                apps=["dummy_app"], wait_for_active=True, status="doesn't matter"
            )
